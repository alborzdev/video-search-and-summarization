...
async def video_report_gen(config: VideoReportGenConfig, builder: Builder) -> AsyncGenerator[FunctionInfo]:
    ...
    vlm_model_name = ""
    try:
        # Builder.get_function_config is synchronous. Awaiting its Pydantic
        # result prevents model capability detection and silently disables the
        # audio prompt guard during workflow registration.
