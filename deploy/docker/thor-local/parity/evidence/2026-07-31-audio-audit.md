...
The released and Thor-derived RT-VLM images are local ARM64 images and contain
the native audio source path. The exact 59-package codec bundle and immutable
networkless audio-codec derivative are now staged. The remaining model artifact
is a pinned Omni snapshot; enabling upstream's proprietary codec install flag
during an offline restart is still prohibited. The smallest intended alternate lane uses the RT-VLM
