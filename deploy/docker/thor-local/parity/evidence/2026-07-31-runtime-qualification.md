...
or VSS-state mutation. The stopped stack produced exactly 24 `unavailable`
probes, zero passed, zero failed, zero skipped, and exit status `2`. This is the
expected stopped-stack baseline and proves the qualifier does not turn absence
into a false pass.
...
local VLM and complete stack can be started and the 24 probes promoted to
