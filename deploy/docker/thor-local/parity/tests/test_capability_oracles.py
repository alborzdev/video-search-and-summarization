...
class CapabilityOracleTests(unittest.TestCase):
    ...
    def test_lvs_mcp_runtime_oracle_is_exact_cleanup_row(self) -> None:
        oracle = next(
            item
            for item in self.plan["oracles"]
        ...
        self.assertEqual(
            oracle["fixture"]["materialization"],
            {
                "path": verifier.LVS_MCP_RUNTIME_FIXTURE["path"],
                "generator": verifier.LVS_MCP_RUNTIME_EXECUTOR,
                "sha256": verifier.LVS_MCP_RUNTIME_FIXTURE["sha256"],
            },
        )
        self.assertEqual(
            oracle["execution_bounds"]["workload"],
            verifier.LVS_MCP_RUNTIME_WORKLOAD,
        )
        self.assertEqual(oracle["execution_bounds"]["max_duration_seconds"], 180)
        self.assertEqual(
            oracle["execution_bounds"]["max_actions"],
            verifier.LVS_MCP_RUNTIME_MAX_ACTIONS,
        )
        self.assertEqual(
            oracle["execution_bounds"]["executor"],
            verifier.LVS_MCP_RUNTIME_EXECUTOR,
        )
        self.assertEqual(
            oracle["execution_bounds"]["collectors"],
            [verifier.LVS_MCP_RUNTIME_VERIFIER],
        )
        self.assertEqual(
            oracle["cleanup"]["targets"], verifier.LVS_MCP_RUNTIME_NAMESPACES
        )
        self.assertEqual(
