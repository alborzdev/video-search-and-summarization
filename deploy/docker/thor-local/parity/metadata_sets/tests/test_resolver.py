...
def test_checked_in_500_set_resolves_as_selected_snapshot() -> None:
    ...
    assert snapshot.set_id == "thor-vss-3.2.1-current-lvs-mcp-runtime-500"
...
def test_checked_in_current_289_set_is_explicitly_resolvable() -> None:
    snapshot = resolver.resolve_metadata_set(
        "thor-vss-3.2.1-current-lvs-mcp-runtime-289"
    )
    assert snapshot.set_id == "thor-vss-3.2.1-current-lvs-mcp-runtime-289"
    assert dict(snapshot.expected_counts) == {
        "capabilities": 289,
        "oracles": 289,
        "feature_families": 55,
    ...
    assert selected.set_id == "thor-vss-3.2.1-current-lvs-mcp-runtime-500"
