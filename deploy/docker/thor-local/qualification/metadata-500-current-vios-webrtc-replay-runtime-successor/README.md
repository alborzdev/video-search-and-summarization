# Current 500-row VIOS WebRTC replay runtime successor

This deterministic successor combines the current 289-row canonical Thor
ledger—including the oracle-bound native VIOS WebRTC replay qualification—with
the unchanged 211 reviewed planning candidates from the prior 500-row view.
Candidate IDs, order, planning-only states, and evidence-empty status remain
unchanged.
...
```bash
python3 deploy/docker/thor-local/qualification/metadata-500-current-vios-webrtc-replay-runtime-successor/project.py --write
python3 deploy/docker/thor-local/qualification/metadata-500-current-vios-webrtc-replay-runtime-successor/project.py
