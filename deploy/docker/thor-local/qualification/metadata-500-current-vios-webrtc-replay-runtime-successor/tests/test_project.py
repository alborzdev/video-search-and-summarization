...
SPEC = importlib.util.spec_from_file_location(
    "metadata_500_current_vios_webrtc_replay_project", HERE / "project.py"
)
...
def test_projection_is_current_prefix_plus_unchanged_candidate_suffix() -> None:
    ...
    replay = next(
        ...
        if row["id"] == "protocol.vios.webrtc-replay"
    )
    assert replay["runtime_state"] == "passed_current"
    assert replay["thor_state"] == "wired"
    oracle = next(
        row
        for row in oracles["oracles"]
        if row["capability_id"] == "protocol.vios.webrtc-replay"
    )
