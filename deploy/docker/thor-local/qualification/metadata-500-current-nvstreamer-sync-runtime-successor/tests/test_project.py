...
SPEC = importlib.util.spec_from_file_location(
    "metadata_500_current_nvstreamer_sync_project", HERE / "project.py"
)
...
def test_projection_is_exact_current_prefix_plus_unchanged_candidate_suffix() -> None:
    ...
    sync = next(
        ...
        if row["id"] == "configuration.nvstreamer.sync"
    )
    assert sync["runtime_state"] == "passed_current"
    assert sync["thor_state"] == "wired"
    oracle = next(
        row
        for row in oracles["oracles"]
        if row["capability_id"] == "configuration.nvstreamer.sync"
    )
