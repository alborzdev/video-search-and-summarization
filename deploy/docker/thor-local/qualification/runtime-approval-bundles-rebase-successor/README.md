# Thor runtime approval bundles rebase successor

This immutable sibling rebases the finalized 16-bundle successor onto the finalized candidate mapping, activation receipt, migration proof, and current candidate-oracle provenance. After normalizing only its contract ID and provenance fields, the entire contract equals the historical successor exactly: bundle IDs, order, dependency DAG, placeholders, tokens, policies, blockers, and zero authorization/execution state are unchanged.
...
PYTHONDONTWRITEBYTECODE=1 python3 \
  deploy/docker/thor-local/qualification/runtime-approval-bundles-rebase-successor/compiler.py \
  --check
...
PYTHONDONTWRITEBYTECODE=1 python3 -m pytest -q -p no:cacheprovider \
  deploy/docker/thor-local/qualification/runtime-approval-bundles-rebase-successor/tests
```

Tests are static and temporary-file-only. They additionally prove provenance-only equivalence to the byte-locked historical successor, the activation-to-migration receipt chain, canonical selector/descriptor preservation, and fail-closed historical/source/package identities.
