...
This package now deterministically compiles both the future receipt-authority oracle
and the package-local Stage-2 promotion projection for:
...
The projected `ledger_binding` is the reviewed final state (`wired`, `passed_current`,
no known gap). The oracle itself remains `current_state: open_unexecuted` with
`evidence: []`; the separate clean aggregate receipt supplies runtime authority.
The compiler derives proposed root-289 and current-500 ledgers, manifests, metadata
descriptors, selector, and two nested official receipts without mutating canonical
metadata files.
...
`--write` is reserved for reviewed deterministic regeneration after an intentional
source/interface lock change. Both ledger rows point to the same aggregate receipt
digest with distinct `/capability_results/0` and `/capability_results/1` selectors.
The Warehouse sample bundle remains excluded and no dependency-pin parity is claimed.
