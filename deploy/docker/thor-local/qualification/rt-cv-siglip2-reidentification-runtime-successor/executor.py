#!/usr/bin/env python3
"""Isolated Thor proof for SigLIP2 appearance re-identification."""
...
def _verify_tokenizer(contract: dict[str, Any], root: Path) -> None:
    ...
    if actual != sorted(expected):
        raise QualificationError("SigLIP2 tokenizer file set drifted")
    for name, identity in expected.items():
        ...
        if (
            ...
            raise QualificationError(f"SigLIP2 tokenizer identity drifted: {name}")


def _verify_static(contract: dict[str, Any]) -> None:
    if contract["official_indices"] != [378] or contract["capability_ids"] != [
        "manifest-entry.rt-cv-2d.03-siglip2-re-identification"
    ]:
        raise QualificationError("official SigLIP2 row binding drifted")
    ...
    if not tokenizer.is_dir() or tokenizer.is_symlink():
        raise QualificationError("SigLIP2 tokenizer directory drifted")
...
for offset in sorted(set(first_cycle).intersection(second_cycle)):
    pre=[obj for obj in first_cycle[offset] if obj["type"]=="Person" and len(obj["embedding"])==1152]
    post=[obj for obj in second_cycle[offset] if obj["type"]=="Person" and len(obj["embedding"])==1152]
    for left in pre:
...
result={
    ...
    "first_cycle_person_embedding_count":sum(1 for rows in first_cycle.values() for obj in rows if obj["type"]=="Person" and len(obj["embedding"])==1152),
    "second_cycle_person_embedding_count":sum(1 for rows in second_cycle.values() for obj in rows if obj["type"]=="Person" and len(obj["embedding"])==1152),
...
def _start_consumer(contract: dict[str, Any], docker_count: list[int]) -> None:
    ...
    run = CORE._docker(
        [
            ...
            "host",
            "--label",
            "vss.thor.qualifier=rt-cv-siglip2-reid",
            "-e",
    ...
    if len(run.stdout.decode().strip()) != 64:
        raise QualificationError("SigLIP2 consumer identity invalid")
    ...
    while time.monotonic() < deadline:
        ...
        if row.get("State", {}).get("Running") is not True:
            raise QualificationError("SigLIP2 consumer exited before assignment")
        time.sleep(0.5)
    raise QualificationError("SigLIP2 consumer did not become ready")
...
def _finish_consumer(contract: dict[str, Any], docker_count: list[int]) -> dict[str, Any]:
    ...
    if waited.returncode != 0 or waited.stdout.decode().strip() != "0" or len(results) != 1:
        ...
        raise QualificationError("SigLIP2 re-entry oracle failed: "+_canonical(summary).decode())
    result=results[0]
...
def _configuration_oracle(contract: dict[str, Any], docker_count: list[int]) -> dict[str, Any]:
    ...
    expected={
        ...
        "text_onnx_path":"/opt/storage/siglip_v2_v1.1.onnx",
        "tokenizer_path":"/opt/storage/siglip_v2_v1.1_tokenizer/",
        "vision_backend":"tensorrt",
        "vision_onnx_path":"/opt/storage/siglip_v2_v1.1.onnx",
        "vision_engine_path":"/opt/storage/siglip_v2_v1.1.onnx_batch16.plan",
    ...
    if any(result[key]!=value for key,value in expected.items()):
        raise QualificationError("effective SigLIP2 configuration drifted")
    result["exact_siglip2_bundle_selected"]=True
...
def _wait_active_metrics(endpoint: str, http_count: list[int], deadline: float) -> dict[str, Any]:
    ...
    while time.monotonic() < deadline:
        ...
        time.sleep(0.5)
    raise QualificationError("active SigLIP2 metrics failed: " + _canonical(last).decode())
...
def _execute(contract: dict[str, Any]) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="rt-cv-siglip2-reid-") as temp_dir:
        temp_path=Path(temp_dir)
        negative=_negative_tokenizer_preflight(contract,temp_path)
        fixture_path=temp_path/"siglip2-reentry.mp4"
        ...
        try:
            ...
            run=CORE._docker([
                ...
                "--label","vss.thor.qualifier=rt-cv-siglip2-reid",
                "-e","DS_MESSAGE_RATE=1","-e","DS_SHOW_SENSOR_ID=false","-e","VISION_ENCODER_MODEL=siglip_v2","-e","VISION_ENCODER_VERSION=v1.1",
            ...
            if len(run.stdout.decode().strip())!=64:
                raise QualificationError("isolated SigLIP2 container identity invalid")
            ...
            media=CORE._docker(["run","-d","--name",execution["mediamtx_container"],"--label","vss.thor.qualifier=rt-cv-siglip2-reid","-p",f"127.0.0.1:{execution['rtsp_port']}:8554",contract["images"]["mediamtx"]["ref"]],docker_count)
            ...
            sensor="siglip2-reentry-runtime"; name="SigLIP2 Reentry Runtime"; url=f"rtsp://127.0.0.1:{execution['rtsp_port']}/siglip2-reentry"
            ...
            if state.get("Running") is not True or state.get("OOMKilled") is not False or row.get("RestartCount")!=0:
                raise QualificationError("isolated SigLIP2 runtime state drifted")
            logs=CORE._docker(["logs",execution["container"]],docker_count).stdout
            runtime_log_sha=hashlib.sha256(logs).hexdigest()
            if b"siglip_v2_v1.1.onnx_batch16.plan" not in logs or b"VPI_ERROR" in logs:
                raise QualificationError("SigLIP2 engine load log or VPI cleanliness failed")
        finally:
            if stream_added:
                try:
                    CORE._stream_call(execution["endpoint"],"remove","siglip2-reentry-runtime","SigLIP2 Reentry Runtime",f"rtsp://127.0.0.1:{execution['rtsp_port']}/siglip2-reentry",http_count)
                    stream_added=False
                except Exception:
    ...
    receipt={
        ...
        "artifact_identity":{"model":"siglip_v2","version":"v1.1","embedding_dimension":1152,"onnx_sha256":contract["assets"]["siglip_onnx"]["sha256"],"weights_sha256":contract["assets"]["siglip_weights"]["sha256"],"engine_sha256":contract["assets"]["siglip_engine"]["sha256"],"tokenizer_file_count":len(contract["tokenizer"]["files"]),"tokenizer_bundle_exact":True},
...
def _plan(contract: dict[str, Any]) -> dict[str, Any]:
    _verify_static(contract)
    return {"schema_version":1,"package_id":contract["package_id"],"official_indices":contract["official_indices"],"status":"ready","exact_siglip2_bundle_present":True,"writes_or_lifecycle_actions":False}
