...
SPEC = importlib.util.spec_from_file_location(
    "metadata_500_current_agent_websocket_project", HERE / "project.py"
)
...
def test_projection_is_current_prefix_plus_unchanged_candidate_suffix() -> None:
    ...
    capability = next(
        ...
        if row["id"] == "protocol.agent.websocket"
    )
    oracle = next(
        row
        for row in oracles["oracles"]
        if row["capability_id"] == "protocol.agent.websocket"
    )
    ...
    assert oracle["execution_bounds"]["max_requests"] == 2
