#!/usr/bin/env python3
"""Verify retained always-on evidence, official-row binding, and source locks."""
...
def main() -> None:
    ...
    assert contract["official_indices"] == [331]
    assert contract["capability_id"] == "manifest-entry.realtime-alerts.03-always-on-lifecycle"
    ...
    assert contract["execution"]["alert_bridge_restart_allowed"] is True
    ...
    assert receipt["always_on"]["worker_created_counts"] == [1, 1, 2, 2, 2]
    assert receipt["always_on"]["worker_removed_counts"] == [0, 0, 1, 1, 2]
