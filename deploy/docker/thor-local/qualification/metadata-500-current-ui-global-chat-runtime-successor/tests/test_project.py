...
SPEC = importlib.util.spec_from_file_location(
    "metadata_500_current_ui_global_chat_project", HERE / "project.py"
)
...
def test_projection_is_current_prefix_plus_unchanged_candidate_suffix() -> None:
    ...
    capability = next(
        ...
        if row["id"] == "runtime.ui.global-chat-sidebar"
    )
    oracle = next(
        row
        for row in oracles["oracles"]
        if row["capability_id"] == "runtime.ui.global-chat-sidebar"
    )
    ...
    assert oracle["execution_bounds"]["max_requests"] == 1200
    assert oracle["execution_bounds"]["max_actions"] == 40
    assert oracle["cleanup"]["targets"] == []
    assert oracle["cleanup"]["allowlist"] == []
