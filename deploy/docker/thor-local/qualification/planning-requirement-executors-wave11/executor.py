#!/usr/bin/env python3
"""Deterministic, fail-closed, read-only Wave 11 static-subset audit."""
...
EXPECTED_INVENTORY_SHA256 = (
    "734609e5b1aab27a4d65a2570d3e6a3c366ae9ecf896dd7f0d5e62785e219cdd"
)
EXPECTED_INVENTORY_SCHEMA_SHA256 = (
    "67d66851da013f32c928727f35bffd49157b28bb481dfd3f35bd0eab5dd01244"
)
EXPECTED_RESULT_SCHEMA_SHA256 = (
    "6e260bbcb6d705932e4a196a395a1962bcc4b28db2f9a330b0fc7f099ea94fbd"
)
MAX_BYTES = 5_000_000
ACCEPTANCE_PATH = (
...
EXPECTED_CASES = {
    "wave11-source-case.kibana-static-configuration": (
        "kibana-availability",
        "runtime.ui.dashboard-tab",
        "static_configuration_subset_only",
    ),
    "wave11-source-case.alert-workflow-modes": (
        "systems-alert-workflow-modes",
        "runtime.alerts.workflow-modes",
        "static_protocol_subset_only",
    ),
    "wave11-source-case.alert-persistence-output": (
        "systems-alert-persistence",
        "runtime.alerts.persistence-output",
        "static_protocol_subset_only",
    ),
    "wave11-source-case.behavior-pipeline-surface": (
        "systems-behavior-pipeline",
        "runtime.behavior.pipeline",
        "static_protocol_subset_only",
    ),
    "wave11-source-case.lvs-queue-surface": (
        "systems-lvs-queue",
        "runtime.lvs.single-request-queue",
        "static_protocol_subset_only",
    ),
    "wave11-source-case.lvs-format-gap": (
        "systems-lvs-formats",
...
EXPECTED_PRIOR_SHA256 = (
    "9ce2f8de129be68b105e6d6da9df0bcfe22f4b4035de0f7689e74f9dd1b3b773"
)
EXPECTED_REMAINDER_SHA256 = (
    "e08aada14a3335ab53b1e15214b85cb72f0df42891fc6c8d62472e4a323fe42d"
)
EXPECTED_SELECTED_SHA256 = (
    "04c47a18aae3ce92b78596fcd1fbd814269f6cf36b6445670ae0af73d259f280"
)
...
def _load_inventory() -> dict[str, Any]:
    for path, expected, label in (
        ...
        if file_sha256(path) != expected:
            raise QualificationError(f"Wave 11 {label} identity drifted")
    ...
    if {row["path"] for row in inventory["baseline_locks"]} != EXPECTED_BASELINE_PATHS:
        raise QualificationError("thirteen-file Wave 11 baseline lock set drifted")
    ...
    if {case["case_id"] for case in cases} != set(EXPECTED_CASES):
        raise QualificationError("six-case Wave 11 case set drifted")
    for case in cases:
        binding = (
...
def build_result() -> dict[str, Any]:
    ...
    if (
        len(prior_selected) != 74
        or canonical_sha256(sorted(prior_selected)) != EXPECTED_PRIOR_SHA256
    ):
        raise QualificationError("exact 74-case predecessor selection set drifted")
    ...
    if len(prior_materialized) != 26 or len(prior_open) != 48:
        raise QualificationError("prior 74 selection composition drifted")
    ...
    if (
        len(remainder_ids) != 35
        or canonical_sha256(sorted(remainder_ids)) != EXPECTED_REMAINDER_SHA256
    ):
        raise QualificationError("exact Wave 10 remainder35 drifted")
    selected = {
    ...
    if (
        ...
        raise QualificationError("Wave 11 selected set drifted")
    if any(
    ...
    if (
        ...
        raise QualificationError("exact Wave 11 remainder-after set drifted")
    ...
    result = {
        ...
        "set_digests": {
            "prior_74": canonical_sha256(sorted(prior_selected)),
            "wave10_remaining_35": canonical_sha256(sorted(remainder_ids)),
            "wave11_selected_6": canonical_sha256(sorted(selected)),
            "wave11_remaining_29": canonical_sha256(sorted(after_ids)),
        },
        ...
        "counts": {
            ...
            "remaining_requirements_audited": 35,
            "cases": 6,
            ...
            "remaining_requirements_unselected": 29,
        },
