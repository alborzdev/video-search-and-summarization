#!/usr/bin/env python3
"""Deterministic, fail-closed, read-only Wave 9 static-subset audit."""
...
EXPECTED_INVENTORY_SHA256 = (
    "ef61deabad56286d7a49c74a5938c0ad2e980c42cb92ff7b0d2df6250c3a6537"
)
...
EXPECTED_CASES = {
    "wave9-source-case.report-persistence-negative": (
        "systems-report-persistence",
        "runtime.base.report-persistence",
        "static_negative_contract_preserved",
    ),
    "wave9-source-case.shared-gpu-configuration": (
        "systems-shared-gpu-memory",
        "deployment.base.shared-gpu-memory",
        "static_configuration_subset_only",
    ),
    "wave9-source-case.va-mcp-seven-tool-surface": (
        "va-mcp-semantic-fixture",
        "api.core.va-mcp-7",
        "static_protocol_subset_only",
    ),
    "wave9-source-case.video-analytics-operation-surface": (
        "smartcity-config-uploads",
        "api.core.video-analytics-56",
        "static_protocol_subset_only",
    ),
    "wave9-source-case.orchestrator-nine-tool-surface": (
        "orchestrator-lifecycle-oracle",
        "api.orchestrator-mcp.tools-9",
        "static_protocol_subset_only",
    ),
    "wave9-source-case.optional-kafka-configuration": (
        "systems-va-optional-kafka",
...
def _load_inventory() -> dict[str, Any]:
    for path, expected, label in (
        ...
        if file_sha256(path) != expected:
            raise QualificationError(f"Wave 9 {label} identity drifted")
    ...
    if {row["path"] for row in inventory["baseline_locks"]} != EXPECTED_BASELINE_PATHS:
        raise QualificationError("eleven-file Wave 9 baseline lock set drifted")
    ...
    if {case["case_id"] for case in cases} != set(EXPECTED_CASES):
        raise QualificationError("six-case Wave 9 case set drifted")
    for case in cases:
        binding = (
...
def build_result() -> dict[str, Any]:
    ...
    if (
        len(prior_selected) != 62
        or canonical_sha256(sorted(prior_selected)) != EXPECTED_PRIOR_SHA256
    ):
        raise QualificationError("exact 62-case predecessor selection set drifted")
    ...
    if len(prior_materialized) != 26 or len(prior_open) != 36:
    ...
    if (
        ...
        raise QualificationError("Wave 9 selected set drifted")
    if any(
