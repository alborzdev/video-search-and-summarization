...
SPEC = importlib.util.spec_from_file_location(
    "metadata_500_current_alert_websocket_project", HERE / "project.py"
)
...
def test_projection_is_current_prefix_plus_unchanged_candidate_suffix() -> None:
    ...
    capability = next(
        row for row in ledger["capabilities"] if row["id"] == "protocol.alert.websocket"
    )
    ...
    oracle = next(
        ...
        if row["capability_id"] == "protocol.alert.websocket"
    )
