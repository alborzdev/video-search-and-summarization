...
def _module():
    spec = importlib.util.spec_from_file_location(
        "wave7_executor", HERE / "executor.py"
    )
...
def test_exact_cases_are_disjoint_from_all_predecessors():
    ...
    assert len(prior) == len(set(prior)) == 50
...
def test_every_binding_identity_and_evidence_class_fails_closed(tmp_path, monkeypatch):
    ...
    for field in (
        ...
        inventory = json.loads(json.dumps(original))
        inventory["cases"][0][field] = "0" * 64
        _use(module, monkeypatch, _write(tmp_path, inventory))
        with pytest.raises(module.QualificationError, match="binding lock mismatch"):
            module.build_result()
    inventory = json.loads(json.dumps(original))
    inventory["cases"][0]["evidence_class"] = "static_configuration_subset_only"
    _use(module, monkeypatch, _write(tmp_path, inventory))
