#!/usr/bin/env python3
"""Compile the immutable SpatialAI 01/04/05 producer-rebind projection."""
...
CANONICAL_BASE_COMMIT = "548f7fdda9148b3ee521c09dcdb298309f25fe2b"
CURRENT_ORACLES = (
...
SELECTED_ORACLES = (
    "deploy/docker/thor-local/qualification/metadata-500-current-spatial-ai-utils-core-successor/post-state-capability-oracles.json",
    "c2b8d584b4bb00f42d6337bbad31038d016fe02a7cfbf92ac747f42075bdc6c0",
)
SELECTED_LEDGER = (
    "deploy/docker/thor-local/qualification/metadata-500-current-spatial-ai-utils-core-successor/post-state-official-capabilities.json",
...
INTERFACE = (
    "deploy/docker/thor-local/qualification/metadata-500-current-spatial-ai-utils-core-rebind-successor/runtime-interface.json",
    "37a3d29e7a83f2bfa6c09c2a41508eb6c785f8b8267117e82f6ebfdcd1edd305",
)
INTERFACE_SCHEMA = (
    "deploy/docker/thor-local/qualification/metadata-500-current-spatial-ai-utils-core-rebind-successor/runtime-interface.schema.json",
    "5c08e40ff22e6101f20fa7938199a90763d1eab668edb156ae479a2061dcac83",
)
...
def load_interface() -> tuple[dict[str, Any], dict[str, dict[str, Any]]]:
    ...
    if (
        producer["canonical_base_commit"] != CANONICAL_BASE_COMMIT
        or producer["source_binding"] != "raw_sha256_current_checkpoint"
    ):
        raise ProjectionError("producer source-binding base drift")
...
def validate(
    ...
    if changed:
        raise ProjectionError("producer rebind changed canonical oracle rows")
    if any(
    ...
    return {
        ...
        "preserved_oracle_rows": len(after),
        "retained_executor_ready": len(TARGET_IDS),
