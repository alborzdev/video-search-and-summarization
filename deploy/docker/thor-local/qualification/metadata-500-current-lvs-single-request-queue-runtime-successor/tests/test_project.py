...
SPEC = importlib.util.spec_from_file_location(
    "metadata_500_current_lvs_single_request_project", HERE / "project.py"
)
...
def test_projection_is_current_prefix_plus_unchanged_candidate_suffix() -> None:
    ...
    capability = next(
        ...
        if row["id"] == "runtime.lvs.single-request-queue"
    )
    oracle = next(
        row
        for row in oracles["oracles"]
        if row["capability_id"] == "runtime.lvs.single-request-queue"
    )
    assert capability["runtime_state"] == "passed_current"
    assert capability["thor_state"] == "wired"
    assert capability["contract"]["batch_queue"] == "external_required"
    assert capability["contract"]["simultaneous_video_processing"] is False
    assert oracle["acceptance_readiness"] == {
        "classification": "executor_ready",
        "blockers": [],
    }
    assert oracle["execution_bounds"]["max_requests"] == 150
