...
SPEC = importlib.util.spec_from_file_location(
    "metadata_500_current_nvstreamer_full_config_project", HERE / "project.py"
)
...
def test_projection_is_exact_current_prefix_plus_unchanged_candidate_suffix() -> None:
    ...
    full_config = next(
        ...
        if row["id"] == "configuration.nvstreamer.full-contract"
    )
    assert full_config["runtime_state"] == "passed_current"
    assert full_config["thor_state"] == "wired"
    oracle = next(
        row
        for row in oracles["oracles"]
        if row["capability_id"] == "configuration.nvstreamer.full-contract"
    )
