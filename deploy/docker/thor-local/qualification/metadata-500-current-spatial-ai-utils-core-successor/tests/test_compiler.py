...
SPEC = importlib.util.spec_from_file_location(
    "spatial_ai_core_oracle_projection", PACKAGE / "compiler.py"
)
...
def test_only_three_oracles_change_and_no_ledger_row_is_promoted(
    derivation: tuple[dict, dict, dict, dict, dict],
) -> None:
    ...
    assert counts == {
        ...
        "preserved_oracle_rows": 497,
        "projected_actions": 21,
        "projected_executor_ready": 3,
...
def test_core_rows_bind_the_exact_committed_runtime_producer(
    derivation: tuple[dict, dict, dict, dict, dict],
) -> None:
    ...
    assert interface["producer"]["commit"] == compiler.PRODUCER_COMMIT
