...
SPEC = importlib.util.spec_from_file_location(
    "metadata_500_current_rt_vlm_sse_project", HERE / "project.py"
)
...
def test_projection_is_current_prefix_plus_unchanged_candidate_suffix() -> None:
    ...
    by_capability = {row["id"]: row for row in ledger["capabilities"]}
    by_oracle = {row["capability_id"]: row for row in oracles["oracles"]}
    for capability_id in RT_VLM_CAPABILITY_IDS:
        assert by_capability[capability_id]["runtime_state"] == "passed_current"
        assert by_capability[capability_id]["thor_state"] == "wired"
        assert by_oracle[capability_id]["acceptance_readiness"] == {
