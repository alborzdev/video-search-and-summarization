...
def test_fifty_three_row_audit_has_six_negative_cases_and_47_unselected():
    ...
    assert len(rows) == len({row["planning_requirement_id"] for row in rows}) == 53
    assert len(selected) == 6
    assert len(rows) - len(selected) == 47
