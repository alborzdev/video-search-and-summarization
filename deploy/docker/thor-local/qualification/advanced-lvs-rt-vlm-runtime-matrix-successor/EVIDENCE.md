...
`matrix.json` binds seventy-five exact VSS 3.2.1 advertised candidate rows in the selected Metadata500 ledger to schema-valid live Thor receipts. Overlay 35 adds indices 60–63. The ready live RT-VLM exposed the URL/header `VlmQuery` fields and rejected loopback SSRF through `/v1/generate_captions`. Its exact released-image download implementation then proved request/environment exact-domain authentication, dangerous-header filtering, no auth over plain HTTP, same-host retention and cross-host stripping, redirect limits across the full 0–10 clamp, per-hop validation, the 8 GiB default plus bounded 64-byte threshold/413 atomicity, and default/listed/unlisted TLS behavior. All fixture traffic was loopback-only; the main container and asset catalog remained exact.
