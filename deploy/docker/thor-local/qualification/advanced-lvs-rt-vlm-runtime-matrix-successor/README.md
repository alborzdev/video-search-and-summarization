...
This read-only successor binds thirty-seven advertised capability rows to current-Thor live receipts. Overlay 18 adds RT-CV 2D RADIO-CLIP to the thirty-six previously proven rows. The exact signed v1.0 NGC artifact emits finite 1,536-dimensional vectors attached to both Pallet and Person detections across 17 object/track pairs. A real two-cycle RTSP/Kafka/API proof also re-associates one object across different tracker IDs, rejects an incompatible 1,024-dimension admission, and restores every owned resource exactly without mutating the main VSS stream inventory.
