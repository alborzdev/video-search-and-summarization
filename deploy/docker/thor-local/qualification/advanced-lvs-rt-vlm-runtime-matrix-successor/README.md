...
This read-only successor binds fifty-eight advertised candidate rows to current-Thor live receipts. Overlay 25 adds all eight Video Analytics API manifest rows plus the VA-MCP row. The retained HTTP run exercised the exact 56-operation surface and every data-bearing query family; a fresh read-only VA-MCP run exercised all eight query tools over JSON-RPC/SSE with real Thor sensors and incidents. `react_agent` was inventoried but intentionally not invoked. Overlay 24's alert WebSocket row and the prior 49 rows retain their exact evidence. External Slack and audio recording remain explicitly unclaimed.
