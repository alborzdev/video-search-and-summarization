...
This read-only successor binds sixty-four advertised candidate rows to current-Thor live receipts. Overlay 27 adds the exact Behavior Analytics dynamic-configuration and dynamic-calibration rows. NVIDIA's 24 configuration scenarios and seven calibration scenarios all passed against the released arm64 service, followed by runtime probes for the 15-second disk fallback, all three calibration implementations, and immutable existing calibration type. Overlay 26's four Behavior Analytics 2D output rows, plus all earlier rows, retain their exact evidence. External Slack and audio recording remain explicitly unclaimed.
