...
This read-only successor binds sixty-two advertised candidate rows to current-Thor live receipts. Overlay 26 adds four exact Behavior Analytics manifest rows: events/incidents, trajectories plus occupancy, directional tripwire/ROI events, and proximity/clustering. The released arm64 service processed a complete 20,000-frame, 171,436-object Kafka replay from three calibrated cameras and emitted real protobuf outputs. Overlay 25's complete Video Analytics API and read-only VA-MCP surfaces, plus all earlier rows, retain their exact evidence. External Slack and audio recording remain explicitly unclaimed.
