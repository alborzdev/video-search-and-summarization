#!/usr/bin/env python3
"""Deterministic, fail-closed, read-only Wave 12 static-subset audit."""
...
EXPECTED_INVENTORY_SHA256 = (
    "360e657f7aeab4b5f37ce45047443976e6b7c7136d5b3363c6fe07e2cff2226a"
)
EXPECTED_INVENTORY_SCHEMA_SHA256 = (
    "6c4177e0ae4ff64bcdaf65d273c1e1713153e716f00a75401c74faaec2fde5ff"
)
EXPECTED_RESULT_SCHEMA_SHA256 = (
    "fd5192185c593e8d6633429381d076dba6ef2294c987b71853e69ce3eb21d9e4"
)
MAX_BYTES = 5_000_000
ACCEPTANCE_PATH = (
...
PRIOR_INVENTORY_PATHS = tuple(
    ...
    for path in (
        ...
        "deploy/docker/thor-local/qualification/planning-requirement-executors-wave10/inventory.json",
        "deploy/docker/thor-local/qualification/planning-requirement-executors-wave11/inventory.json",
    )
)
...
EXPECTED_CASES = {
    "wave12-source-case.search-routing-ranking": (
        "search-documents-and-bboxes",
        "runtime.agent.search-profile",
        "static_protocol_subset_only",
    ),
    "wave12-source-case.realtime-alert-surface": (
        "tiny-alert-stream",
        "runtime.workflow.real-time-alerts",
        "static_protocol_subset_only",
    ),
    "wave12-source-case.alerts-ui-configuration": (
        "ui-alert-api",
        "runtime.ui.alerts-tab",
        "static_configuration_subset_only",
    ),
    "wave12-source-case.search-ui-configuration": (
        "ui-search-api",
        "runtime.ui.search-tab",
        "static_configuration_subset_only",
    ),
    "wave12-source-case.video-management-ui-configuration": (
        "ui-tiny-media",
        "runtime.ui.video-management-tab",
        "static_configuration_subset_only",
    ),
    "wave12-source-case.alert-worker-backpressure": (
...
EXPECTED_PRIOR_SHA256 = (
    "127fd31fa4d7cfc7b84115e08500db2d93137eda3f8160468ae371d1128648e5"
)
EXPECTED_REMAINDER_SHA256 = (
    "e86a4e00bd319b4fd822d0bbf4502e1ea4bfc45122efd73ddb9a488e8375ff0f"
)
EXPECTED_SELECTED_SHA256 = (
    "0b77dc52e1347fffd454f55519702c72a4637905ee00f868cbdce5fc760d33fe"
)
EXPECTED_AFTER_SHA256 = (
    "244182f25fed82f0f9e481563901fb48d87a1e61b24042a76ba7897920d562c9"
)
...
def _load(path: Path, label: str | None = None) -> Any:
    ...
    return _strict_json(_read_regular(path, name), name)
...
def _load_inventory() -> dict[str, Any]:
    for path, expected, label in (
        ...
        if file_sha256(path) != expected:
            raise QualificationError(f"Wave 12 {label} identity drifted")
    ...
    if {row["path"] for row in inventory["baseline_locks"]} != EXPECTED_BASELINE_PATHS:
        raise QualificationError("fourteen-file Wave 12 baseline lock set drifted")
    ...
    if {case["case_id"] for case in cases} != set(EXPECTED_CASES):
        raise QualificationError("six-case Wave 12 case set drifted")
    for case in cases:
        binding = (
...
def build_result() -> dict[str, Any]:
    ...
    if (
        len(prior_selected) != 80
        or canonical_sha256(sorted(prior_selected)) != EXPECTED_PRIOR_SHA256
    ):
        raise QualificationError("exact 80-case predecessor selection set drifted")
    ...
    if len(prior_materialized) != 26 or len(prior_open) != 54:
        raise QualificationError("prior 80 selection composition drifted")
    ...
    if (
        len(remainder_ids) != 29
        or canonical_sha256(sorted(remainder_ids)) != EXPECTED_REMAINDER_SHA256
    ):
        raise QualificationError("exact Wave 11 remainder29 drifted")
    selected = {
    ...
    if (
        ...
        raise QualificationError("Wave 12 selected set drifted")
    if any(
    ...
    if (
        ...
        raise QualificationError("exact Wave 12 remainder-after set drifted")
    ...
    for case in inventory["cases"]:
        ...
        for lock in case["source_locks"]:
            path = _safe_repo_file(lock["path"])
            try:
                texts[lock["path"]] = _read_regular(
                    path, f"repository source {lock['path']}"
                ).decode("utf-8")
            except UnicodeDecodeError as exc:
                raise QualificationError(
                    f"non-UTF-8 assertion source in {case['case_id']}"
                ) from exc
        assertion_results = []
        for assertion in case["assertions"]:
    ...
    result = {
        ...
        "set_digests": {
            "prior_80": canonical_sha256(sorted(prior_selected)),
            "wave11_remaining_29": canonical_sha256(sorted(remainder_ids)),
            "wave12_selected_6": canonical_sha256(sorted(selected)),
            "wave12_remaining_23": canonical_sha256(sorted(after_ids)),
        },
