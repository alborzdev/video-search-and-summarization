"""Adversarial tests for the Metadata-500 migration rebase successor."""
...
SPEC = importlib.util.spec_from_file_location(
    "live_metadata_500_migration_rebase_successor_compiler", PACKAGE / "compiler.py"
)
...
def compiled():
    pending = compiler.pending_final_locks()
    if pending:
        pytest.skip("final composition and output hashes are intentionally unpinned")
...
def _successor_output_paths():
    return {
        compiler.LEDGER_OUTPUT,
        compiler.MANIFEST_OUTPUT,
        compiler.ACCEPTANCE_OUTPUT,
        compiler.ORACLE_OUTPUT,
        compiler.ORACLE_SCHEMA_OUTPUT,
        compiler.PROOF_OUTPUT,
        compiler.PROOF_SCHEMA_OUTPUT,
    }


def test_predecessor_package_is_exactly_hash_locked():
    documents, payloads, locks = compiler._load_predecessors()
    assert set(documents) == set(compiler.PREDECESSOR_INPUTS)
    assert set(payloads) == set(compiler.PREDECESSOR_INPUTS)
    assert set(locks) == set(compiler.PREDECESSOR_INPUTS)
    for source_id, (path, expected) in compiler.PREDECESSOR_INPUTS.items():
        assert locks[source_id]["path"] == path
        assert locks[source_id]["raw_sha256"] == expected
        assert _sha(payloads[source_id]) == expected
        assert locks[source_id]["git_blob_oid"] == _blob(payloads[source_id])


def test_successor_outputs_never_alias_predecessor_paths():
    predecessor_paths = {
        compiler._repo_file(path).resolve()
        for path, _ in compiler.PREDECESSOR_INPUTS.values()
    }
    successor_paths = {path.resolve() for path in _successor_output_paths()}
    assert not successor_paths & predecessor_paths
    assert all(path.parent == PACKAGE for path in successor_paths)


def test_current_selected_pair_remains_exactly_locked():
    _, payloads, locks = compiler._load_predecessors()
    assert locks["selector"]["raw_sha256"] == (
        "8021efc4684b106539a627b3ddfd82937c7ecf1d6111a3c87538fc5ea7360bec"
    )
    assert locks["descriptor"]["raw_sha256"] == (
        "56ed8f555c0814e80a39c15198a33c80d400c991a4857a27b4609ea88b5750f9"
    )
    assert _sha(payloads["selector"]) == locks["selector"]["raw_sha256"]
    assert _sha(payloads["descriptor"]) == locks["descriptor"]["raw_sha256"]


def test_package_writer_rejects_predecessor_and_canonical_paths():
    for source_id in ("proof", "manifest", "selector", "descriptor"):
        path, _ = compiler.PREDECESSOR_INPUTS[source_id]
        with pytest.raises(compiler.MigrationError, match="outside the isolated package"):
            compiler._atomic_write(compiler._repo_file(path), b"blocked")


def test_predecessor_and_canonical_hashes_unchanged_after_package_local_write(
    tmp_path, monkeypatch
):
    before = {
        source_id: _sha(compiler._repo_file(path).read_bytes())
        for source_id, (path, _) in compiler.PREDECESSOR_INPUTS.items()
    }
    monkeypatch.setattr(compiler, "PACKAGE", tmp_path)
    compiler._atomic_write(tmp_path / compiler.PROOF_OUTPUT.name, b"local-only")
    after = {
        source_id: _sha(compiler._repo_file(path).read_bytes())
        for source_id, (path, _) in compiler.PREDECESSOR_INPUTS.items()
    }
...
def test_final_lock_state_is_explicit_and_fail_closed_when_pending():
    pending = compiler.pending_final_locks()
    if pending:
        with pytest.raises(compiler.MigrationError, match="not pinned"):
            compiler.compile_migration()
        return
    proof, schema, outputs = compiler.compile_migration()
    compiler.validate_migration(proof, schema, outputs)
    compiler._validate_expected(proof, outputs)


def test_applied_selector_and_descriptor_semantically_bind_predecessor():
    documents, _, locks = compiler._load_predecessors()
    binding = compiler._validate_applied_selection(documents, locks)
    assert binding["set_id"] == "thor-vss-3.2.1-metadata-500-staged"
    assert binding["selector_raw_sha256"] == locks["selector"]["raw_sha256"]
    assert binding["descriptor_raw_sha256"] == locks["descriptor"]["raw_sha256"]

    selector_attack = copy.deepcopy(documents)
    selector_attack["selector"]["selected_set"] = "thor-vss-3.2.1-live-289"
    with pytest.raises(compiler.MigrationError, match="selector"):
        compiler._validate_applied_selection(selector_attack, locks)

    descriptor_attack = copy.deepcopy(documents)
    descriptor_attack["descriptor"]["documents"]["official_capabilities"][
        "raw_sha256"
    ] = "0" * 64
    with pytest.raises(compiler.MigrationError, match="official_capabilities"):
        compiler._validate_applied_selection(descriptor_attack, locks)
...
def test_wrong_pin_rejects_write_before_transaction(monkeypatch):
    ...
    monkeypatch.setitem(compiler.EXPECTED, "ledger", "0" * 64)
    assert compiler.main(["--write"]) == 1
    assert called is False


def test_transactional_writer_rolls_back_all_members_on_commit_failure(
    tmp_path, monkeypatch
):
    monkeypatch.setattr(compiler, "PACKAGE", tmp_path)
    paths = {tmp_path / name for name in compiler._allowed_output_names()}
    outputs = {path: f"new:{path.name}".encode() for path in paths}
    before = {path: f"old:{path.name}".encode() for path in paths}
    ...
    def fail_third_staged_commit(source, target):
        nonlocal staged_commit_count
        if ".stage." in Path(source).name:
            staged_commit_count += 1
            if staged_commit_count == 3:
                raise OSError("injected commit failure")
        return real_replace(source, target)

    monkeypatch.setattr(compiler.os, "replace", fail_third_staged_commit)
    with pytest.raises(compiler.MigrationError, match="prior state restored"):
...
def test_journal_before_after_hashes_and_reverse_rollback(compiled):
    ...
    for row in journal:
        before = compiler._repo_file(row["predecessor_path"]).read_bytes()
        after = compiler._repo_file(row["successor_path"]).read_bytes()
        assert row["changed"] is (before != after)
    ...
    assert [
        row["predecessor_path"] for row in proof["rollback"]["reverse_targets"]
    ] == [
        row["predecessor_path"] for row in reversed(journal)
    ]
...
def test_policy_excludes_warehouse_and_live_apply(compiled):
    ...
    assert proof["policy"]["modifies_predecessor_package"] is False
    assert proof["policy"]["modifies_selector_or_descriptors"] is False
    assert proof["policy"]["creates_git_patch"] is False
    assert proof["rollback"]["live_apply_authorized"] is False
    assert (
        proof["application_boundary"]["id"]
        == "selector-descriptor-rebase-successor-required"
    )
