...
SPEC = importlib.util.spec_from_file_location(
    "metadata_500_current_nvstreamer_project", HERE / "project.py"
)
...
def test_projection_is_exact_current_prefix_plus_unchanged_candidate_suffix() -> None:
    ...
    nvstreamer = next(row for row in ledger["capabilities"] if row["id"] == "runtime.nvstreamer.file-streaming")
    assert nvstreamer["runtime_state"] == "passed_current"
    assert nvstreamer["thor_state"] == "wired"
