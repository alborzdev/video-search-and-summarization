...
"""Remove public STUN dependencies from a private Sparse4D snapshot."""
...
RELATIVE_CONFIGS = (
    Path("industry-profiles/warehouse-operations/warehouse-3d-app/vst/configs/vst_config.json"),
    Path("industry-profiles/warehouse-operations/warehouse-3d-app/nvstreamer/configs/vst-config.json"),
)


def main() -> int:
    parser = argparse.ArgumentParser(description="Patch private Sparse4D VST configs offline")
    ...
    for relative in RELATIVE_CONFIGS:
        ...
        if target.is_symlink() or not target.is_file():
            raise SystemExit(f"Sparse4D VST config is missing or is a symlink: {target}")
        document = json.loads(target.read_text(encoding="utf-8"))
        network = document.get("network")
        if not isinstance(network, dict) or "stunurl_list" not in network:
            raise SystemExit(f"Sparse4D VST config has no network.stunurl_list: {target}")
