...
grep -q "Ledger: 54 families, 385 advertised capabilities, 16 skills" <<<"${report}"
grep -q "Thor state: external_optional=7, partial=28, source_only=6, wired=13" <<<"${report}"
grep -q "Runtime: blocked=1, not_applicable=7, not_qualified=36, passed_current=2, passed_prior=6, static_only=2" <<<"${report}"
...
grep -q "infra-observability: wired/not_qualified" <<<"${report}"
